...
class Client
{
     ...
     * @throws ClientException Throw on API return invalid response.
     */
    public function postOrderRefund($orderId, $refundId, $amount,$receipt=array() , $baseUrl=self::PAYMENTS_API_URI)
    { 
        $url = $baseUrl.'/orders/'.$orderId.'/refunds/'.$refundId;         
        return $this->requestBuilder($url, self::POST, [ 'amount' => $amount,"receipt"=>$receipt ]);
